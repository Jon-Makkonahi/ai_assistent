...
async def get_user_by_telegram_id(telegram_id: str, db: AsyncSession = Depends(get_async_db)):
    """
    Получает пользователя по его telegram_id.
    """
    user = await get_user_by_telegram_id_service(telegram_id, db)
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден")
    return user
...
async def update_user(user_id: int, user_data: UserCreate, db: AsyncSession = Depends(get_async_db)):
    """
